...
<div class="site">
	...
	<div id="main">
		...
		<div style="margin:80px 240px 60px 240px;">
			<hr style="height:2px;border:none;border-top:2px solid gray;">
			<div style="overflow-x: auto;">
				<table style="color: gray;width:2000px; font-size: 18px;">
					...
					<c:forEach var="order" items="${listOrder}">
						<tr style="font-size: 18px;height:36px;">
							...
							<td>
								<c:if test="${order.flag==1}">Approve</c:if>
								<c:if test="${order.flag==2}">Waiting</c:if>
								<c:if test="${order.flag==3}">Refuse</c:if>
								<c:if test="${order.flag==4}">Logistics</c:if>
								<c:if test="${order.flag==5}">Receiving</c:if>
								<c:if test="${order.flag==6}">Complete</c:if>
							</td>
							<td>${order.createdate}</td>
							<td>${order.updatedate}</td>
							<td><a href="deleteOrder?id=${order.id}" style="color:green;">Delete</a></td>
							<c:if test="${order.flag!=6}">
								<c:if test="${order.flag<4}">
									<td><a href="approveOrder?id=${order.id}" style="color:green;">Approve</a></td>
									<td><a href="refuseOrder?id=${order.id}" style="color:green;">Refuse</a></td>
								</c:if>
								<c:if test="${order.flag==4}">
									<td><a href="logistics?id=${order.id}" " style="color:green;">Logistics</a></td>
									<td><a href="sendRe?id=${order.id}" style="color:green;">Receive</a></td>
								</c:if>
							</c:if>
						</tr>
			       		<%srno++;%>
